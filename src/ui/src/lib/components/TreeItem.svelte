<script lang="ts">
  ...
  const isDirPartial = $derived(
    node.is_directory && node.selection_state === "partial"
  );
  const isDirFull = $derived(
    node.is_directory && node.selection_state === "full"
  );
  ...
  >
    ...
    <div class="name-and-button">
      ...
      <button
        ...
        data-type="ignore"
      >
        <svg class="icon ignore-icon" viewBox="0 0 24 24"
