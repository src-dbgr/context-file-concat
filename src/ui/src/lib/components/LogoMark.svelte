  ...
  <svg viewBox="0 0 24 24" class="svg" aria-hidden="true" focusable="false">
    <defs>
      ...
      <filter id="boltBloom" x="-50%" y="-50%" width="200%" height="200%">
        ...
        <feMerge
          ><feMergeNode in="b2" /><feMergeNode in="b1" /><feMergeNode
            in="SourceGraphic"
          /></feMerge
        >
      </filter>
...
<style>
  ...
  .cfc-logo.is-striking .svg {
    animation:
      cfc-strike-strobe 0.9s cubic-bezier(0.2, 0.75, 0.2, 1) both,
  ...
  .cfc-logo.is-striking .bolt-stroke {
    ...
    opacity: 1;
    filter: drop-shadow(0 0 calc(1px + 1px * var(--amp)) currentColor)
  ...
  .flash-ring {
    fill: none;
    stroke: var(--flash);
    stroke-width: calc(0.6px + 0.6px * var(--amp));
    opacity: 0;
  ...
  .cfc-logo::after {
    ...
    background:
      ...
      radial-gradient(
        closest-side,
        color-mix(in oklab, var(--flash) 60%, transparent),
        transparent 75%
      );
    mix-blend-mode: screen;
    filter: blur(calc(0.3px + 0.5px * var(--amp)));
